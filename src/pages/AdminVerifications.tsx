  ...
  return (
    <AdminLayout>
      ...
      <div className="rounded-sm border px-5 pt-6 pb-2.5 shadow-default border-strokedark bg-boxdark sm:px-7.5 xl:pb-1">
        <h2 className="font-bold text-xl mb-5 p-4 text-white rounded-md ">ALL USERS</h2>
          ...
          <table className="w-full table-auto ">
            <thead>
              <tr className=" text-left bg-meta-4 ">
                ...
                <th className="min-w-[100px] py-4 px-4 font-medium text-white dark:text-white">Document</th>
                <th className="min-w-[100px] py-4 px-4 font-medium text-white dark:text-white">Status</th>
            ...
            <tbody>
              {paginatedUsers?.map((userItem: any, key: number) => (
                <tr key={key}>
                  ...
                  <td className="border py-5 px-4 border-strokedark">
                    <h5 className="font-medium text-white dark:text-white">{userItem.fullname}</h5>
                  </td>
                  ...
                  <td className="border py-5 px-4 border-strokedark">
                    <p
                      className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${
                        userItem.status === "verified" ? "text-success bg-success" : "text-warning bg-warning"
                      }`}
                    >
