...
const AdminDeposit = (props: Props) => {
  ...
  return (
    <AdminLayout>
      {showModal && (
        <Modal show={showModal} closeModal={closeModal} title="User IDentification" height={450} width={400}>
          <div className="flex items-center justify-center">
            <img src={viewImg} alt="user deposit upload" className="h-[300px]" />
          </div>
        </Modal>
      )}
      <Modal show={showRemoveDepositModal} closeModal={closeModal} title="Delete Deposit" height={270} width={400}>
      ...
      <div className="rounded-sm border px-5 pt-6 pb-2.5 shadow-default border-strokedark bg-boxdark sm:px-7.5 xl:pb-1">
        <h2 className="font-bold text-xl mb-5 p-4 text-white rounded-md ">ALL USERS DEPOSIT</h2>
          ...
          <table className="w-full table-auto">
            <thead>
              <tr className=" text-left bg-meta-4 ">
                ...
                <th className="min-w-[160px] py-4 px-4 font-medium text-white dark:text-white">Payment Method</th>
                <th className="min-w-[100px] py-4 px-4 font-medium text-white dark:text-white">Amount</th>
                <th className="min-w-[100px] py-4 px-4 font-medium text-white dark:text-white">Screenshot</th>
            ...
            <tbody>
              {paginatedUsers?.map((deposit: DepositState, key: number) => (
                <tr key={key}>
                  ...
                  <td className="border py-5 px-4 border-strokedark">
                    <h5 className="font-medium text-white dark:text-white">{deposit.fullname}</h5>
                  </td>

                  <td className="border py-5 px-4 border-strokedark">
                    <p className="text-white dark:text-white">{deposit.method}</p>
                  </td>
                  <td className="border py-5 px-4 border-strokedark">
                    <p className="text-white dark:text-white">${deposit.amount}</p>
                  </td>
                  <td className="border py-5 px-4 border-strokedark">
                    <button
                  ...
                  <td className="border py-5 px-4 border-strokedark">
                    <p className="text-white dark:text-white">{deposit.date}</p>
                  </td>

                  <td className="border py-5 px-4 border-strokedark">
                    <p
                      className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium ${
                        deposit.status === "completed" ? "text-success bg-success" : "text-warning bg-warning"
                      }`}
                    >
                  ...
                  {deposit.status === "pending" && (
                    <td className="border py-5 px-4 flex items-center gap-x-2 border-strokedark">
                      <button
                        onClick={() => handleUpdateDepositStatus(deposit.id || "", deposit.uid || "")}
                      ...
                      <button
                        onClick={() => openRemoveDepositModal(deposit.id || "", deposit.uid || "")}
